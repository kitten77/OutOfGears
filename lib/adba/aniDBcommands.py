...
from lib.adba.aniDBresponses import *
...
class ProducerCommand(Command):
    ...
    def cached(self, intr, db):
        ...
        if len(rows) > 1:
            raise AniDBInternalError(
                "It shouldn't be possible for database to return more than 1 line for PRODUCER cache")
        elif not len(rows):
            return None
        else:
...
class MyListCommand(Command):
    ...
    def cached(self, intr, db):
        ...
        if lid:
            ...
            if resp.rescode != '220':
                resp = NoSuchFileResponse(self, None, '321', 'NO SUCH ENTRY (FILE NOT FOUND)', [])
                resp.parse()
                return resp
...
class MyListDelCommand(Command):
    def __init__(self, lid=None, fid=None, aid=None, aname=None, gid=None, gname=None, epno=None):
        if not (lid or fid or ((aid or aname) and (gid or gname) and epno)) or (
            ...
            raise AniDBIncorrectParameterError(
                "You must provide <lid+edit=1 XOR fid XOR a(id|name)+g(id|name)+epno> for MYLISTDEL command")
